...
const sketch = p5 => {
  ...
  p5.draw = () => {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      var mag1 = Math.sqrt(
        Math.pow(lines[i].x - p5.mouseX, 2) +
          Math.pow(lines[i].y - p5.mouseY, 2)
      );
      var mag2 = Math.sqrt(
        Math.pow(lines[i].px - p5.mouseX, 2) +
          Math.pow(lines[i].py - p5.mouseY, 2)
      );
      var mult = 9 + i / 10;
      ...
      p5.line(
        (lines[i].x * (1 + mag1) + mult * p5.mouseX) / div1,
